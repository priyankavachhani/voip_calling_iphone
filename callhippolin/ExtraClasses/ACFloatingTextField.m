...
- (CGRect)textRectForBounds:(CGRect)bounds {
    return CGRectMake(35, 0, bounds.size.width, bounds.size.height);
}
...
-(void)setimage
{
    ...
    envelopeView.image = _SelectedImage;
    envelopeView.contentMode = UIViewContentModeScaleAspectFit;
    UIView *test=  [[UIView alloc]initWithFrame:CGRectMake(0, 0, 45, 30)];
    [test addSubview:envelopeView];
    [self.leftView setFrame:envelopeView.frame];
    self.leftView =test;
...
-(void)addPlaceholderLabel{
    ...
    _labelPlaceholder.frame = CGRectMake(90, 0.0, self.frame.size.width - 40, self.frame.size.height);
...
-(void)addErrorPlaceholderLabel{
    ...
    self.labelErrorPlaceholder.translatesAutoresizingMaskIntoConstraints = NO;
     self.labelErrorPlaceholder.frame = CGRectMake(90, 0.0, self.frame.size.width - 40, self.frame.size.height);
