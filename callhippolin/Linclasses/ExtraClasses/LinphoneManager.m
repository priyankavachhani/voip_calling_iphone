...
//    [core setBool:NO forKey:@"account_ice_preference"];
 //   [core setBool:NO forKey:@"ice_preference"];
   ...
   /* NSString *username = @"";
    ...
    while (username && proxies &&
        ...
        if (proxies) {
            ...
            NSLog(@"ICE state :: %d",linphone_nat_policy_ice_enabled(policy));
            
            if (policy) {
                NSLog(@"ice disable");
                linphone_nat_policy_enable_ice(policy, NO);
            }
...
//            }else
//            {
//                linphone_core_enable_payload_type(lc,pt,NO);
//            }
//
            if(strcmp([pref UTF8String], "pcmu_preference") == 0) {
                NSLog(@"Codec Settings %@",pref);
    ...
    if ([self lpConfigBoolForKey:@"avpf_migration_done"] == FALSE) {
        ...
        while (proxies) {
            ...
            if (addr &&
                ...
                linphone_proxy_config_avpf_enabled(proxy);
                //linphone_proxy_enable
                }
            proxies = proxies->next;
        }
...
- (void)setPushNotificationToken:(NSData *)apushNotificationToken {
    ...
    @try {
        ...
        NSLog(@"s: linphone core not ready yet, ignoring push token");
    }
}
...
//                                    kLinphoneAudioVbrCodecDefaultBitrate);
    
    int bitrate = linphone_config_get_int(
                                    _configDb, "audio", "codec_bitrate_limit",
                                    kLinphoneAudioVbrCodecDefaultBitrate);
    /*default value is in linphonerc or linphonerc-factory*/
