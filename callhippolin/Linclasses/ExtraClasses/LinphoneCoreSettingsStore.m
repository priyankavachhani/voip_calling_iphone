...
//        [self setBool:linphone_nat_policy_ice_enabled(np) forKey:@"ice_preference"];
        [self setBool:NO forKey:@"ice_preference"];
//        [self setBool:linphone_nat_policy_turn_enabled(np) forKey:@"turn_preference"];
        [self setBool:NO forKey:@"turn_preference"];
...
//        int random_port_preference = [lm lpConfigIntForKey:@"random_port_preference" withDefault:1];
        int random_port_preference = [lm lpConfigIntForKey:@"random_port_preference" withDefault:0];
...
- (void)synchronizeAccounts {
    ...
    if (username && [username length] > 0 && domain && [domain length] > 0) {
        ...
        if (linphone_proxy_config_set_identity_address(proxyCfg, identity) == -1) {
...
//        linphone_proxy_config_set_nat_policy(proxyCfg, policy);
        ...
        linphone_proxy_config_avpf_enabled(proxyCfg);
       // linphone_proxy_config_enable_avpf(proxyCfg, use_avpf);
        ...
        if (from) {
            ...
            if (password) {
                ...
                NSLog(@"register failed core setting if");
                ...
                NSLog(@"register failed core setting else");
