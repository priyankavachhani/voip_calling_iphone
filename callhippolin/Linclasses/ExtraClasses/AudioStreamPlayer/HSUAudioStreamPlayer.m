...
@implementation HSUAudioStreamPlayer
{
    ...
    UInt32 _bufferPacketsCounts[kMaxBufferQueueSize];
        ...
        @autoreleasepool {
            if (_seekByteOffset) {
                ...
                if (!error && !(ioFlags & kAudioFileStreamSeekFlag_OffsetIsEstimated)) {
                    ...
                    _seekByteOffset = (NSUInteger)packetAlignedByteOffset + _dataStartOffset;
                }
                if (error) {
                    HLogErr(error);
                }
...
- (void)_computeBufferQueueSize
{
    ...
    if (_bufferByteSize &&
        ...
        if (_bufferByteSize) {
            HLog(@"bufferByteSize invalid, use default %lu", _bufferSize * _bufferQueueSize);
        }
